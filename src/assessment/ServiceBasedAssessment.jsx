  ...
  const calculateResults = () => {
    ...
    (async () => {
        try {
            ...
            // 1. Bulk Gap Analysis
            const result = await TeacherAgent.analyzeGap(questionsDB, answers, topic);
            setAnalysis(result.analysis || result.knowledge_gap);
            // Optionally set strategy if it ever gets added to state
                        ...
                        {(() => {
                           ...
                           return (
                                <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/40 p-6 text-center">
                                    ...
                                    <p className="text-xs font-bold text-white uppercase tracking-widest mb-1">Preview Unavailable</p>
                                    <p className="text-[10px] text-slate-500 leading-relaxed px-4">Direct video preview could not be loaded. Please use the 'Start Preparation' link to view the material.</p>
                                </div>
                           );
                        })()}
                      </div>
