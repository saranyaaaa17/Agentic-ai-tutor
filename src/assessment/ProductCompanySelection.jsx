...
const ProductCompanySelection = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050B14] text-white p-8 md:p-12 font-sans selection:bg-yellow-500/30 relative">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="inline-block mb-4 px-4 py-1 rounded-full border border-yellow-500/30 bg-yellow-500/10 text-yellow-400 text-sm font-mono tracking-widest uppercase">
            Product & FAANG Track
          </div>
          ...
          <p className="text-[#9CA3AF] text-xl max-w-3xl mx-auto leading-relaxed">
            Prepare for high-performance interviews at product-based companies.
            Focus on <span className="text-white font-bold">Data Structures, Algorithms, and System Design</span>.
          </p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {productCompanies.map((id, index) => {
             ...
             <motion.div
               ...
               whileHover={{ y: -5 }}
               className="bg-slate-900/50 border border-white/10 rounded-3xl overflow-hidden hover:border-yellow-500/50 shadow-2xl transition-all group relative backdrop-blur-sm"
             >
                {/* Header Background */}
                <div className={`h-40 bg-linear-to-r ${company.color} relative p-8 overflow-hidden`}>
                   ...
                   <div className="absolute right-0 bottom-0 opacity-20 transform translate-x-1/4 translate-y-1/4">
                        <svg width="200" height="200" viewBox="0 0 200 200" fill="none">
                            <path d="M100 0L200 50V150L100 200L0 150V50L100 0Z" stroke="white" strokeWidth="2"/>
                        </svg>
                   </div>
                   
                   <h2 className="text-4xl font-bold text-white relative z-10 tracking-tight">{company.name}</h2>
                ...
                <div className="p-8">
                   ...
                   <div className="grid grid-cols-2 gap-4 mb-8">
                      <div className="bg-black/30 p-4 rounded-xl border border-white/5">
                        <div className="text-xs text-slate-500 uppercase tracking-widest mb-1">Key Focus</div>
                        <div className="font-bold text-yellow-100 truncate">
                   ...
                   <div className="mb-8 space-y-3">
                        ...
                        {Object.entries(company.domains).map(([key, domain]) => (
                            domain.weight > 0 && (
                                <div 
                                    ...
                                    className={`h-full ${
                                        key.includes('dsa') ? 'bg-red-500' : 
                                        key === 'system_design' ? 'bg-yellow-500' :
                                        key === 'programming' ? 'bg-blue-500' :
                                        key === 'cs' ? 'bg-purple-500' : 'bg-slate-500'
                                    }`}
                                />
                            )
                        ))}
                      ...
                      <div className="flex gap-4 text-xs text-slate-500 mt-2 justify-start font-mono">
                         <span className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-red-500"/>Advanced Algo</span>
                         <span className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-yellow-500"/>System Design</span>
                      </div>
                   </div>
