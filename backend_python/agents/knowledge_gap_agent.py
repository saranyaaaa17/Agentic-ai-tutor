...
class KnowledgeGapAgent:
    ...
    async def _call_llm(self, messages: List[Dict[str, str]], temperature: float = 0.5) -> str:
        ...
        return await llm_service.call_llm(messages, temperature, json_mode=True, agent_name="KnowledgeGapAgent")
    ...
    async def analyze(self, questions: List[Dict], answers: Dict[str, str], topic: str) -> Dict:
        ...
        # Fallback
        from agents.fallback_data import FALLBACK_ANALYSIS
        fallback = FALLBACK_ANALYSIS.copy()
        fallback["mastery_profile"] = concept_scores
        return fallback
